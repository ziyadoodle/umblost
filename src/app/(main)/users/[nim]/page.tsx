...
const getUser = cache(async (nim: string, loggedInUserId: string) => {
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    const user = await getUser(nim, loggedInUserId.id)

    return {
        title: `${user.name} (@${user.nim})`,
    }
}
            ...
            <div className="flex flex-wrap gap-3 sm:flex-nowrap">
                <div className="me-auto space-y-3">
                    <div>
                        ...
                        <div className="text-muted-foreground">@{user.nim}</div>
                    </div>
